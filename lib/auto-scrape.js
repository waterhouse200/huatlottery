...
async function runAll(label = "scheduled") {
  ...
  try {
    ...
    // Announcements ride the same cycle: collect what the operators published,
    // then turn anything new into an article. Both are spawned as their own
    // processes like the scrapers above, so a failure in either can't take the
    // API down, and neither is allowed to fail the run — draw results are the
    // priority and must still be reported if the article pipeline has a bad day.
    const anns = await runOne("announcements.js");
    const arts = anns.ok ? await runOne("generate-articles.js") : { ok: false, code: null };
    lastRun = startedAt.toISOString();
    lastResult = {
      label,
      fourd: { ok: fourd.ok, code: fourd.code },
      toto: { ok: toto.ok, code: toto.code },
      announcements: { ok: anns.ok, code: anns.code },
      articles: { ok: arts.ok, code: arts.code },
    };
    console.log(`[auto-scrape] ${label} done — 4D ok=${fourd.ok} TOTO ok=${toto.ok} announcements ok=${anns.ok} articles ok=${arts.ok}`);
    return lastResult;
  } finally {
    inFlight = false;
  }
