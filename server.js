...
app.get("/sitemap.xml", (req, res) => {
  ...
  const tabs = ["dashboard","fourd","toto","alpha4d","history","prediction","lucky","mynumbers"];
  const urls = [`<url><loc>${SITE_URL}/</loc><lastmod>${today}</lastmod><changefreq>daily</changefreq><priority>1.0</priority></url>`]
    .concat(tabs.map(t => {
      const priority = (t === "fourd" || t === "toto" || t === "dashboard") ? "0.9" : "0.7";
      const freq     = (t === "fourd" || t === "toto" || t === "dashboard") ? "daily" : "weekly";
      return `<url><loc>${SITE_URL}/?tab=${t}</loc><lastmod>${today}</lastmod><changefreq>${freq}</changefreq><priority>${priority}</priority></url>`;
    }));
...
// current gap, lifetime profit, and how much of profit comes from SMALL
// wins (starter $250 + consolation $60) vs rare big wins (1st/2nd/3rd).
// A "sustainable" number drips small hits often enough that the gaps
  ...
  try {
    ...
    rows.forEach((r, idx) => {
      function add(num, tier) {
        ...
        const k = String(num).padStart(4, "0");
        if (!numHistory[k]) numHistory[k] = [];
        numHistory[k].push({ idx, date: r.draw_date, tier, payout: PAY[tier] });
      }
      add(r.first_prize, "first");
      add(r.second_prize, "second");
      add(r.third_prize, "third");
    ...
    for (let n = 0; n < 10000; n++) {
      const k = String(n).padStart(4, "0");
      const history = numHistory[k] || [];
      if (history.length < MIN_HITS) continue;
      const gaps = [];
      for (let i = 1; i < history.length; i++) gaps.push(history[i].idx - history[i - 1].idx);
      const medGap = median(gaps);
      const longestGap = gaps.length ? Math.max(...gaps) : null;
      const minGap = gaps.length ? Math.min(...gaps) : null;
      const currentGap = totalDraws - 1 - history[history.length - 1].idx;
      const tiers = { first: 0, second: 0, third: 0, starter: 0, consol: 0 };
      let revenue = 0, smallRevenue = 0;
      for (const h of history) {
        tiers[h.tier]++; revenue += h.payout;
        if (h.tier === "starter" || h.tier === "consol") smallRevenue += h.payout;
      }
      const lifetimeProfit = revenue - totalDraws;
      const profitFromSmallOnly = smallRevenue - totalDraws;     // would small wins alone cover the $5,493 cost?
      const smallHitsCount = tiers.starter + tiers.consol;
      const smallHitsRatio = Math.round(smallHitsCount / history.length * 100);
      const avgPayout = Math.round(revenue / history.length);
      // Gap distribution bucket
      const gapBuckets = { '<=50': 0, '51-150': 0, '151-300': 0, '301-700': 0, '700+': 0 };
      for (const g of gaps) {
        if (g <= 50) gapBuckets['<=50']++;
        else if (g <= 150) gapBuckets['51-150']++;
        else if (g <= 300) gapBuckets['151-300']++;
        else if (g <= 700) gapBuckets['301-700']++;
        else gapBuckets['700+']++;
      }
      results.push({
        number: k,
        total_hits: history.length,
        hits_per_year: Number((history.length / totalDraws * 156).toFixed(2)),
        median_gap: medGap,
        longest_gap: longestGap,
        min_gap: minGap,
        current_gap: currentGap,
        last_hit_date: history[history.length - 1].date,
        last_hit_tier: history[history.length - 1].tier,
        tiers,
        small_hits: smallHitsCount,
        small_hits_pct: smallHitsRatio,
        avg_payout: avgPayout,
        lifetime_profit: lifetimeProfit,
        profit_from_small_only: profitFromSmallOnly,
        gap_buckets: gapBuckets,
      });
    }
    // Sort by smallest median gap (steadiest)
    const steadiest = results.slice().sort((a, b) => a.median_gap - b.median_gap || b.total_hits - a.total_hits).slice(0, 5);
    // Sort by lifetime profit (overall best)
    const mostProfitable = results.slice().sort((a, b) => b.lifetime_profit - a.lifetime_profit).slice(0, 5);

    // === iBet analysis: group by sorted digits ===
    ...
    const ibetHistory = {};                  // sortedKey → combined history
    const ibetPerms   = {};                  // sortedKey → Set of distinct perms seen
    for (const k of Object.keys(numHistory)) {
      const sortedKey = k.split("").sort().join("");
      if (!ibetHistory[sortedKey]) { ibetHistory[sortedKey] = []; ibetPerms[sortedKey] = new Set(); }
      ibetHistory[sortedKey] = ibetHistory[sortedKey].concat(numHistory[k]);
    ...
    const ibetResults = [];
    for (const sk of Object.keys(ibetHistory)) {
      const hist = ibetHistory[sk].slice().sort((a, b) => a.idx - b.idx);
      if (hist.length < MIN_HITS) continue;
      const gaps = [];
      for (let i = 1; i < hist.length; i++) gaps.push(hist[i].idx - hist[i - 1].idx);
      const medGap = median(gaps);
      const longestGap = gaps.length ? Math.max(...gaps) : null;
      const currentGap = totalDraws - 1 - hist[hist.length - 1].idx;
      const tiers = { first:0, second:0, third:0, starter:0, consol:0 };
      let revenue = 0, smallRevenue = 0;
      for (const h of hist) {
        tiers[h.tier]++; revenue += h.payout;
      ...
      const nPerms = uniquePerms(sk);
      // iBet: revenue divided by number of unique permutations
      const ibetRev = revenue / nPerms;
      const ibetSmallRev = smallRevenue / nPerms;
      const lifetimeProfit = Math.round(ibetRev - totalDraws);
      const profitFromSmallOnly = Math.round(ibetSmallRev - totalDraws);
      const smallHitsCount = tiers.starter + tiers.consol;
      const smallHitsRatio = Math.round(smallHitsCount / hist.length * 100);
      const avgPayout = Math.round(ibetRev / hist.length);
      // Gap distribution
      const gapBuckets = { '<=50': 0, '51-150': 0, '151-300': 0, '301-700': 0, '700+': 0 };
      for (const g of gaps) {
        if (g <= 50) gapBuckets['<=50']++;
      ...
      ibetResults.push({
        ...
        perms_count: nPerms,
        total_hits: hist.length,
        hits_per_year: Number((hist.length / totalDraws * 156).toFixed(2)),
        median_gap: medGap,
        longest_gap: longestGap,
        current_gap: currentGap,
        last_hit_date: hist[hist.length - 1].date,
        last_hit_tier: hist[hist.length - 1].tier,
        tiers,
        small_hits: smallHitsCount,
        small_hits_pct: smallHitsRatio,
        avg_payout: avgPayout,
    ...
    const ibetSteadiest = ibetResults.slice()
      .sort((a, b) => a.median_gap - b.median_gap || b.total_hits - a.total_hits)
      .slice(0, 5);
    const ibetMostProfitable = ibetResults.slice()
    ...
    res.json({ success: true, data: {
      ...
      total_ibet_candidates: ibetResults.length,
      steadiest,
      most_profitable: mostProfitable,
      ibet_steadiest: ibetSteadiest,
